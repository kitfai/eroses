...
class ExtractedPendaftaranTable(Base):
    __tablename__ = 'extracted_pendaftaran'
...
class TextractCustomQueriesAdapter:
    ...
    def analyze_document_with_adapter(
        ...
        try:
            ...
            response = self.textract_client.start_document_analysis(
                ...
                QueriesConfig={
                    'Queries': [
                        {'Text': "what is the Nama Pertubuhan"},
                        {'Text': "What is alamat berdaftar"},
                        {'Text': "what is the Tarikh Kuatkuasa"},
                        {'Text': "what is the Nombor Pendaftaran"},
    ...
    def _process_blocks(self, blocks: List[Dict], results: Dict):
        ...
        query_mapping = {
            'what is the Tarikh Kuatkuasa': 'organization_date',
...
class PartyDirectoryProcessor:
    ...
    def process_sijil_dir(self, base_path: str, year: str,party_dir: str, party_info:PartyName):
        ...
        for sijil_type_dir in directories:
            if 'pendaftaran' in sijil_type_dir.lower():
                self.process_pendaftaran_dir(base_path, year, sijil_type_dir,party_dir, party_info)  # process bendera type
                # break
            else:
                continue

    def process_pendaftaran_dir(self, base_path: str, year: str, sijil_name: str,party_dir: str, party_info:PartyName):
    ...
    def process_pendaftaran_pdf(self, path: str, base_path: str, year: str, pdf: str,party_dir: str, party_info:PartyName):
        ...
        adapter_id = '3b8ef74389bf'  # Replace with your actual adapter ID
        ...
        try:
            ...
            pendaftaranData = ExtractedPendaftaranTable()
            key_value = {}
            ...
            for key in self.pendaftaran_keys:
                print(f"Key: {key}")
            ...
            if len(not_found_keys) > 0:
                ...
                unprocessed = UnprocessedData(
                    ...
                    sijil_type=SijilType.PENDAFTARAN.value
            ...
            for key, value in key_value.items():
                if key == 'organization_name':
                    pendaftaranData.nama_pertubuhan = value
                elif key == 'organization_address':
                    pendaftaranData.alamat_berdaftar = value
                elif key == 'organization_date':
                    pendaftaranData.tarikh_berkuatkuasa = value
                elif key == 'ppm_no':
                    pendaftaranData.nombor_ppm = value
                elif key == 'organization_no':
                    pendaftaranData.nombor_pendaftaran = value
                elif key == 'organization_name':
                    pendaftaranData.nama_pertubuhan = value
            pendaftaranData.s3_path = path
            pendaftaranData.party_name = party_dir
            if party_info:
                pendaftaranData.party_id = party_info.id
            pendaftaranData.year_directory = year
            pendaftaranData.filename = pdf
            self.session.add(pendaftaranData)
