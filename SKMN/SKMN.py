...
class ExtractedNamaTable(Base):
    ...
    nama_asal_pertubuhan = Column(String(150))
    nama_baharu_pertubuhan = Column(String(150))
...
class TextractCustomQueriesAdapter:
    ...
    def analyze_document_with_adapter(
        ...
        try:
            ...
            response = self.textract_client.start_document_analysis(
                ...
                QueriesConfig={
                    'Queries': [
                        {'Text': "What is Nama Asal Pertubuhan?"},
                        {'Text': "What is Nama Baharu Pertubuhan?"},
    ...
    def _process_blocks(self, blocks: List[Dict], results: Dict):
        ...
        '''
        query_mapping = {
            'What is Nama Asal Pertubuhan?': 'organization_original_name',
            'What is Nama Baharu Pertubuhan?': 'organization_new_name',
...
class PartyDirectoryProcessor:
    ...
    def process_sijil_specific_party(self, party_dir: str, party_info: PartyName):
        ...
        base_path = f"{self.root_directory}/{party_dir}/Cawangan/Sijil/SKMN"
        result = {
            'directory_name': 'SKMN',
            'subdirectories': {}
        }
    ...
    def process_years(self, base_path: str, party_dir: str, party_info: PartyName):
        ...
        for year in directories:  # process every year
            ...
            self.process_nama_dir(base_path, year, party_dir, party_info)
    def process_sijil_dir(self, base_path: str, year: str, party_dir: str, party_info: PartyName):
        ...
        for sijil_type_dir in directories:
            if 'nama' in sijil_type_dir.lower():
                self.process_nama_dir(base_path, year, sijil_type_dir, party_dir, party_info)  # process bendera type
                # break
            else:
                continue

    def process_nama_dir(self, base_path:str, year:str, party_dir:str, party_info:PartyName):
        ...
        for pdf in pdfs:
            try:
                ...
                self.process_nama_pdf(f"{base_path}/{year}/{pdf}", base_path, year, pdf, party_dir,party_info)
            except Exception as e:
                logger.error(f"Error in process_nama_dir process: {str(e)}")

    def process_nama_pdf(self, path: str, base_path: str, year: str, pdf: str, party_dir: str, party_info: PartyName):
        region_name = 'ap-southeast-1'
        analyzer = TextractCustomQueriesAdapter(region_name=region_name)
        adapter_id = '49c443652696'  # Replace with your actual adapter ID
        ...
        try:
            ...
            if len(not_found_keys) > 0:
                ...
                unprocessed = UnprocessedData(
                    ...
                    sijil_type=SijilType.NAMA.value
            ...
            for key, value in key_value.items():
                if key == 'organization_original_name':
                    namaData.nama_asal_pertubuhan = value
                elif key == 'organization_new_name':
                    namaData.nama_baharu_pertubuhan = value
                elif key == 'organization_date':
                    namaData.tarikh_berkuatkuasa = value
                elif key == 'ppm_no':
                    namaData.nombor_ppm = value
                elif key == 'organization_no':
                    namaData.nombor_pendaftaran = value
            namaData.s3_path = path
            namaData.year_directory = year
            namaData.filename = pdf
            namaData.party_name = party_dir
            if party_info:
                namaData.party_id = party_info.id

            self.session.add(namaData)
...
def main():
    ...
    db_connection = "mysql+mysqlconnector://root:admin@localhost:3306/eroses"
    ...
    # ori processor.process_sijil_specific_party("BARISAN KEMAJUAN INDIA SE-MALAYSIA")
