...
class ExtractedUndangTable(Base):
    __tablename__ = 'extracted_undang'
...
class PartyDirectoryProcessor:
    ...
    def process_years(self, base_path:str, party_name:str,party_info:PartyName):
        ...
        for year in directories: #process every year
            self.process_sijil_dir(base_path, year,party_name,party_info) #check for bendera directory

    def process_sijil_dir(self,base_path:str, year:str, party_name:str,party_info:PartyName):
        directories = self.navigator.list_directories(f"{base_path}/{year}/")  # ['years']


        for sijil_type_dir in directories:
            if 'undang' in sijil_type_dir.lower() :
                self.process_undang_dir(base_path,year,sijil_type_dir, party_name,party_info) #process bendera type
                #break
            else:
                continue
    ...
    def process_undang_dir(self, base_path:str, year:str, sijil_name:str,party_name:str,party_info:PartyName):
    ...
    def process_undang_pdf(self,path:str,base_path:str, year:str, pdf:str,party_name:str,party_info:PartyName):
        ...
        adapter_id = 'd2e89c632515'  # Replace with your actual adapter ID
        ...
        try:
            ...
            key_value = {}
            undangData = ExtractedUndangTable()
            undangData.party_name = party_name
            if party_info:
                undangData.party_id = party_info.id
            ...
            if len(not_found_keys) > 0:
                ...
                unprocessed = UnprocessedData(
                    ...
                    sijil_type=SijilType.UNDANG.value
            ...
            for key, value in key_value.items():
                if key == 'organization_name':
                    undangData.nama_pertubuhan = value
                elif key == 'organization_address':
                    undangData.alamat_berdaftar = value
                elif key == 'organization_date':
                    undangData.tarikh_berkuatkuasa = value
                elif key == 'ppm_no':
                    undangData.nombor_ppm = value
                elif key == 'organization_no':
                    undangData.nombor_pendaftaran = value


            undangData.s3_path = path
            undangData.year_directory = year
            undangData.filename = pdf
            self.session.add(undangData)
