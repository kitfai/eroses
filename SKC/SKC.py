...
class ExtractedAlamatTable(Base):
    ...
    alamat_cawangan = Column(String(1000))
    nama_cawangan = Column(String(1000))
    party_name = Column(String(150), nullable=True)
    nombor_ppm = Column(String(50))
    nombor_cawangan = Column(String(50))
...
class TextractCustomQueriesAdapter:
            ...
            response = self.textract_client.start_document_analysis(
                ...
                QueriesConfig={
                    'Queries': [
                        ...
                        {'Text': "What is Nama Cawangan?"},
                        {'Text': "What is Alamat Cawangan?"},
                        {'Text': "What is Nombor Cawangan?"},
    ...
    def _process_blocks(self, blocks: List[Dict], results: Dict):
        ...
        query_mapping = {
            ...
            'What is Nama Cawangan?': 'cawangan_name',
            'What is Alamat Cawangan?': 'organization_address',
            "What is Nombor Cawangan?": 'cawangan_no',
...
class PartyDirectoryProcessor:
    ...
    def process_parties(self, base_path:str):
        ...
        #parties_directories = ["PARTI MAJU SABAH"]
        # Process each directory
    ...
    def process_sijil_specific_party(self, party_dir: str, party_info:PartyName) :
        ...
        base_path = f"{self.root_directory}/{party_dir}/Cawangan/Sijil/SKC"
        result = {
            'directory_name': 'SKC',
            'subdirectories': {}
        }
    ...
    def process_years(self, base_path:str, party_dir:str ,party_info:PartyName):
        ...
        for year in directories: #process every year
            ...
            self.process_skc_dir(base_path, year, party_dir, party_info)
    ...
    def process_sijil_dir(self,base_path:str, year:str , party_dir:str, party_info:PartyName):
        ...
        for sijil_type_dir in directories:
            if 'SKC' in sijil_type_dir.lower() :
                self.process_alamat_dir(base_path,year,sijil_type_dir, party_dir, party_info) #process bendera type
                #break
            else:
                continue

    def process_skc_dir(self, base_path:str, year:str, party_dir:str, party_info:PartyName):
        ...
        for pdf in pdfs:
            try:
                ...
                self.process_skc_pdf(f"{base_path}/{year}/{pdf}", base_path, year, pdf,party_dir, party_info)
            except Exception as e:
                logger.error(f"Error in process_alamat_dir process: {str(e)}")

    def process_skc_pdf(self,path:str, base_path:str, year:str, pdf:str, party_dir:str, party_info:PartyName):
        region_name = 'ap-southeast-1'
        analyzer = TextractCustomQueriesAdapter(region_name=region_name)
        adapter_id = '95e48a0da9e5'  # Replace with your actual adapter ID
        ...
        try:
            ...
            for key, value in key_value.items():
                if key == 'organization_name':
                    if isinstance(value, list):
                        # Join all list items into a single string separated by a space
                        alamatData.nama_pertubuhan = ' '.join(value)
                    else:
                        # If it's already a string, assign it directly
                        alamatData.nama_pertubuhan = value

                elif key == 'cawangan_name':
                    alamatData.nama_cawangan = value

                elif key == 'organization_address':
                    alamatData.alamat_cawangan = value

                elif key == 'cawangan_no':
                    alamatData.nombor_cawangan = value
